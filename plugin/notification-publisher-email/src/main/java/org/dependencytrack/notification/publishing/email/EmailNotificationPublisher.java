...
final class EmailNotificationPublisher implements NotificationPublisher {
    ...
    private final Cache<@NonNull SessionCacheKey, Session> sessionCache;

    EmailNotificationPublisher(final Cache<@NonNull SessionCacheKey, Session> sessionCache) {
    ...
    public void publish(final PublishContext ctx, final Notification notification) {
        ...
        requireNonNull(ctx.destination(), "destination must not be null");
        ...
        final String messageContent = ctx.templateRenderer().render(ctx.template(), notification);
