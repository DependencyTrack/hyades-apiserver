...
public final class WorkflowEngineInitializer implements ServletContextListener {
    ...
    public void contextInitialized(final ServletContextEvent event) {
        if (!config.getOptionalValue("dt.workflow-engine.enabled", Boolean.class).orElse(false)) {
            return;
        }
    ...
    private static WorkflowEngineConfig createEngineConfig(final Config config) {
        final String dataSourceName = config.getValue("dt.workflow-engine.datasource.name", String.class);
        ...
        config.getOptionalValue("dt.workflow-engine.cache.run-history.max-size", int.class)
                .ifPresent(engineConfig.runHistoryCache()::setMaxSize);
        config.getOptionalValue("dt.workflow-engine.cache.run-history.ttl", Duration.class)
                .ifPresent(engineConfig.runHistoryCache()::setEvictAfterAccess);

        config.getOptionalValue("dt.workflow-engine.buffer.external-event.flush-interval", Duration.class)
                .ifPresent(engineConfig.externalEventBuffer()::setFlushInterval);
        config.getOptionalValue("dt.workflow-engine.buffer.external-event.max-size", int.class)
                .ifPresent(engineConfig.externalEventBuffer()::setMaxBatchSize);

        config.getOptionalValue("dt.workflow-engine.buffer.task-command.flush-interval", Duration.class)
                .ifPresent(engineConfig.taskCommandBuffer()::setFlushInterval);
        config.getOptionalValue("dt.workflow-engine.buffer.task-command.max-size", int.class)
                .ifPresent(engineConfig.taskCommandBuffer()::setMaxBatchSize);

        config.getOptionalValue("dt.workflow-engine.retention.enabled", boolean.class)
