...
public final class ReconcileVulnAnalysisResultsActivity implements Activity<ReconcileVulnAnalysisResultsArg, Void> {
    ...
    private Map<VulnIdAndSource, Long> syncVulnsBatch(
        ...
        useJdbiTransaction(handle -> {
            var vulnDao = new VulnerabilityDao(handle);
            results.putAll(vulnDao.syncMany(convertedVulnByVulnIdAndSource, this::canUpdateVulnerability));
        });
        return results;
    }
    ...
    private void reconcileFindings(
        ...
        // Only evaluate policies for active findings.
    ...
    private record FindingAttributionKey(
        ...
        private static FindingAttributionKey of(FindingDao.FindingAttribution finding) {
