        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            ...
            final Project createdProject = qm.callInTransaction(() -> {
                ...
                if (!chosenTeams.isEmpty()) {
                    ...
                    boolean isAdmin = getEffectivePermissions().contains(Permissions.Constants.ACCESS_MANAGEMENT)
                            || getEffectivePermissions().contains(Permissions.Constants.ACCESS_MANAGEMENT_READ);
        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            final Project updatedProject = qm.callInTransaction(() -> {
                ...
                if (jsonProject.isLatest() && !project.isLatest()) {
                    ...
                    // TODO: Returns null when portfolio ACL is enabled and old latest is not accessible
                    //  by principal. Rewrite this to deal with that fact.
