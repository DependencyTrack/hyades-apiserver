...
public class PermissionResource extends AlpineResource {
    ...
    public Response setUserPermissions(
        ...
        try (QueryManager qm = new QueryManager()) {
            ...
            final List<Permission> requestedPermissions = qm.getPermissions()
                    .stream()
                    .filter(permission -> request.permissions().contains(permission.toString()))
                    .toList();

            final List<Permission> currentPermissions = user.getPermissions();

            if (currentPermissions != null && currentPermissions.equals(requestedPermissions)) {
