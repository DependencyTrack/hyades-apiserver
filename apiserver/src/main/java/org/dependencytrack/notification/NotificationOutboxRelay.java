...
final class NotificationOutboxRelay implements Closeable {
    ...
    private RelayCycleOutcome executeRelayCycle() {
        return inJdbiTransaction(handle -> {
            ...
            final List<NotificationPublishTask> publishTasks = router.route(currentBatch);
    ...
    private void sendAll(Collection<NotificationPublishTask> publishTasks) {
        final var createRunRequests = new ArrayList<CreateWorkflowRunRequest<?>>(publishTasks.size());

        for (final var publishTask : publishTasks) {
            final var workflowArgumentBuilder =
                    PublishNotificationArgument.newBuilder()
                            .setRuleId(publishTask.ruleId())
                            .setRuleName(publishTask.ruleName());

            final Notification notification = publishTask.notification();
            ...
            //
            // 128KiB is a conservative size limit, which we could easily raise
            // or make configurable if needed.
            if (notification.getSerializedSize() > 128 * 1024) {
                LOGGER.warn(
                        "Notification {} exceeds size limit of 128KiB and will be offloaded to file storage",
                        notification.getId());

                try (final var fileStorage = pluginManager.getExtension(FileStorage.class)) {
                    final FileMetadata fileMetadata = fileStorage.store(
                            "notification/%s.proto".formatted(notification.getId()),
                            "application/protobuf",
                            new ByteArrayInputStream(notification.toByteArray()));
                    workflowArgumentBuilder.setNotificationFileMetadata(fileMetadata);
                } catch (IOException e) {
                    throw new UncheckedIOException("Failed to store notification file", e);
                }
            } else {
                workflowArgumentBuilder.setNotification(notification);
            }

            createRunRequests.add(
                    new CreateWorkflowRunRequest<>(PublishNotificationWorkflow.class)
                            .withArgument(workflowArgumentBuilder.build()));
        }
