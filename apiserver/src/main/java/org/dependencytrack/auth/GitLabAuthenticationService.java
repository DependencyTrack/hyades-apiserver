 ...
 public class GitLabAuthenticationService implements AuthenticationService {
     ...
     public Principal authenticate() throws AlpineAuthenticationException {
         ...
         final OidcProfileCreator profileCreator = claims -> {
             ...
             List<String> devGroups = claims.getStringListClaim("https://gitlab.org/claims/groups/developer");
             List<String> maintainerGroups = claims.getStringListClaim("https://gitlab.org/claims/groups/maintainer");
             List<String> ownerGroups = claims.getStringListClaim("https://gitlab.org/claims/groups/owner");
             List<String> groups = new ArrayList<String>();
             if(devGroups != null) {
	             for(String group : devGroups) {
	            	groups.add(group + "_" + "developer");
	             }
             }
             if(maintainerGroups != null) {
	             for(String group : maintainerGroups) {
	             	groups.add(group + "_" + "maintainer");
	              }
             }
             if(ownerGroups != null) {
            	 for(String group : ownerGroups) {
	             	groups.add(group + "_" + "owner");
	             }
             }
             if(!groups.isEmpty())
