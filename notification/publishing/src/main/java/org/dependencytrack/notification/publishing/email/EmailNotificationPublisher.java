...
final class EmailNotificationPublisher implements NotificationPublisher {
    ...
    EmailNotificationPublisher(Cache<SessionCacheKey, Session> sessionCache) {
    ...
    private Session getSession(EmailNotificationRuleConfig ruleConfig) {
        final boolean authenticated =
                ruleConfig.getSmtpUsername() != null
                        && ruleConfig.getSmtpPassword() != null;

        final Properties props = new Properties();
        props.put("mail.smtp.host", ruleConfig.getSmtpHost());
        props.put("mail.smtp.port", ruleConfig.getSmtpPort());
        props.put("mail.smtp.socketFactory.port", ruleConfig.getSmtpPort());
        if (authenticated) {
        ...
        return sessionCache.get(
                new SessionCacheKey(props, Objects.hash(ruleConfig.getSmtpUsername(), ruleConfig.getSmtpPassword())),
                ignored -> {
                        ...
                        authenticator = new Authenticator() {
                            ...
                            protected PasswordAuthentication getPasswordAuthentication() {
                                return new PasswordAuthentication(
                                        ruleConfig.getSmtpUsername(),
                                        ruleConfig.getSmtpPassword());
                            }
                        };
                    }
