...
class EmailNotificationPublisherFactoryTest {
    ...
    @Test
    void extensionNameShouldReturnEmail() {
        try (final var publisherFactory = new EmailNotificationPublisherFactory()) {
            assertThat(publisherFactory.extensionName()).isEqualTo("email");
        }
    }
    ...
    void extensionClassShouldReturnPublisherClass() {
        try (final var publisherFactory = new EmailNotificationPublisherFactory()) {
            assertThat(publisherFactory.extensionClass()).isEqualTo(EmailNotificationPublisher.class);
        }
    }
