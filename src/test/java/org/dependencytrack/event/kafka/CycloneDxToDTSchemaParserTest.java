...
public class CycloneDxToDTSchemaParserTest extends PersistenceCapableTest {

   Bom bom;

    @Before
    public void setup() throws IOException {
        bom = new ObjectMapper().readValue("""
                {
                   "version": 1,
                   "components": [
                     {
                ...
                """, new TypeReference<Bom>() {
        });

    }

    @Test
    public void testVulnParsing(){
        MirrorVulnerabilityProcessor processor = new MirrorVulnerabilityProcessor();
        Vulnerability vuln = processor.mapBomToVulnerability(qm, bom);
    ...
    public void testCalculateSeverity(){
        MirrorVulnerabilityProcessor processor = new MirrorVulnerabilityProcessor();
        Severity severity = processor.calculateOSVSeverity(bom);
