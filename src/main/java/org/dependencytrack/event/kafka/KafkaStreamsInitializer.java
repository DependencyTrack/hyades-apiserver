...
public class KafkaStreamsInitializer implements ServletContextListener {
    ...
    public void contextInitialized(final ServletContextEvent event) {
        ...
        properties.put(StreamsConfig.STATE_DIR_CONFIG, Paths.get(Config.getInstance().getProperty(Config.AlpineKey.DATA_DIRECTORY),
                "kafka-streams", "state").toAbsolutePath().toString());
        properties.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, Config.getInstance().getProperty(ConfigKey.KAFKA_AUTO_OFFSET_RESET));
        STREAMS = new KafkaStreams(new KafkaStreamsTopologyFactory().createTopology(), new StreamsConfig(properties));
