...
public class NistMirrorTask implements LoggableSubscriber {
    ...
    private void getAllFiles() {
        ...
        if (mirroredWithoutErrors) {
            String content = "Mirroring of the National Vulnerability Database completed successfully";
            NotificationUtil.dispatchExceptionNotifications(NotificationScope.SYSTEM, NotificationGroup.DATASOURCE_MIRRORING, NotificationConstants.Title.NVD_MIRROR, content , NotificationLevel.INFORMATIONAL);
        }
    }
        ...
        try {
            ...
            try (final CloseableHttpResponse response = HttpClientPool.getClient().execute(request)) {
                ...
                if (status.getStatusCode() == 200) {
                    ...
                    String content = "An error occurred mirroring the contents of the National Vulnerability Database. Check log for details. HTTP Response: " + status.getStatusCode() + ". " + detailMessage;
                    NotificationUtil.dispatchExceptionNotifications(NotificationScope.SYSTEM, NotificationGroup.DATASOURCE_MIRRORING, NotificationConstants.Title.NVD_MIRROR, content , NotificationLevel.ERROR);
                } else {
                    mirroredWithoutErrors = false;
                    LOGGER.warn("Unable to download - HTTP Response " + status.getStatusCode() + ": " + status.getReasonPhrase());
                    String content = "An error occurred mirroring the contents of the National Vulnerability Database. Check log for details. HTTP Response: " + status.getStatusCode();
                    NotificationUtil.dispatchExceptionNotifications(NotificationScope.SYSTEM, NotificationGroup.DATASOURCE_MIRRORING, NotificationConstants.Title.NVD_MIRROR, content , NotificationLevel.ERROR);
                }
            }
        } catch (IOException e) {
            ...
            String content = "An error occurred mirroring the contents of the National Vulnerability Database. Check log for details. " + e.getMessage();
            NotificationUtil.dispatchExceptionNotifications(NotificationScope.SYSTEM, NotificationGroup.DATASOURCE_MIRRORING, NotificationConstants.Title.NVD_MIRROR, content , NotificationLevel.ERROR);
        }
    }
